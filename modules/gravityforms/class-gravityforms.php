<?php
/**
 * Module Name: Gravity Forms Add-on
 * Description: Integrates <a href="http://www.gravityforms.com/" target="_blank">Gravity Forms</a> with <strong>Infusionsoft®</strong> allowing form submissions to be automatically sent to your account.
 *
 * @link       http://infugrator.com/
 * @since      1.0.0
 * @package    Infugrator
 * @subpackage Infugrator/modules
 * @author     Cosmin Schiopu <sc.cosmin@gmail.com>
 */

class IFG_Gravity_Forms extends IFG_Module{

	/**
	 * @since    1.0.0
	 * @access   public
	 */
	public static $name = 'Gravity Forms Add-on';


	/**
	 *	The slug of module, must be unique and as short as possible
	 *
	 * @since    1.0.0
	 * @access   public
	 */
	public static $slug = 'gravityforms';


	/**
	 * @since    1.0.0
	 * @access   public
	 */
	public static $dependency_name = '<a href="http://www.gravityforms.com/" target="_blank">Gravity Forms</a>';


	/**
	 * @since    1.0.0
	 * @access   public
	 */
	public static $dependency_version = 2210;


	/**
	 * @since    1.0.0
	 * @access   protected
	 */
	protected $mapped_fields = null;


	/**
	 * @since    1.0.0
	 * @access   public
	 */
	public static $instance = null;



	/**
	 * Create a single instance of the class
	 *
	 * @since 1.0.0
	 * @return object
	 */
	public static function instance() {
		if ( is_null( self::$instance ) ) {
			self::$instance = new self();
		}
		return self::$instance;
	}



	/**
	 * Initialize things before running
	 *
	 * @since  1.0.0
	 * @return mixed
	 */
	public function initialize(){

		$this->mapped_fields = $this->utility->wp_option('get', 'ifg-'.self::$slug.'-fields');

		$this->loader->add_filter('ifg_widgets_stats', $this, 'insert_stats_widget');
		$this->loader->add_filter('ifg_stats_data', $this, 'add_stats_data');

		//this will catch the sync stats process and update the data
		$this->loader->add_action('wp_ajax_nopriv_ifg_stats_data', $this, 'add_stats_data');
		$this->loader->add_action('wp_ajax_ifg_stats_data', $this, 'add_stats_data');

		$this->loader->add_action( 'wp_ajax_nopriv_ifg_gravityforms_search_data', $this, 'search_tags');
		$this->loader->add_action( 'wp_ajax_ifg_gravityforms_search_data', $this, 'search_tags');

		$this->loader->add_action('admin_enqueue_scripts', $this, 'admin_assets');

		$this->loader->add_action('gform_form_settings_menu', $this, 'set_form_settings_menu');
		$this->loader->add_action('gform_form_settings_page_gf_infusionsoft', $this, 'output_form_settings_tab');
		$this->loader->add_action('gform_tooltips', $this, 'add_tooltips');
		$this->loader->add_action('gform_after_delete_form', $this, 'update_mapped_fields_on_form_deletion');
		$this->loader->add_action('gform_after_submission', $this, 'insert_contact', 10, 2);

	}



	/**
	 * Run the main processes
	 *
	 * @since 1.0.0
	 */
	protected function run(){

		if($this->check_dependency('gravityforms', self::$dependency_version)){

			$this->save_mapped_fields();

			$this->loader->run();

		}else{

			add_action("admin_notices", array($this, 'dependency_notice'), 10);
		}
	}



	/**
	 * Display dependency messages
	 *
	 * @since  1.0.0
	 * @return string
	 */
	public function dependency_notice() {

		$this->dependency_message(array(
			'module'  => self::$name,
			'slug'    => 'gravityforms',
			'name'    => self::$dependency_name,
			'version' => self::$dependency_version,
		));
    }



	/**
	 * Instert widget in statistics page
	 *
	 * @since  1.0.0
	 * @param  array $widgets
	 * @return array
	 */
	public function insert_stats_widget($widgets){

		$output = array_merge($widgets, array(
			array(
				'id'      => 'ifg-'.self::$slug,
				'title'   => __('Captured Contacts by Gravity Forms', 'infugrator'),
				'content' => '<i class="fa fa-spinner fa-spin"></i>',
			),
		));

		return $output;
	}



	/**
	 * Add data for the widget
	 *
	 * @since 1.0.0
	 * @param array
	 * @return array
	 */
	public function add_stats_data($stats){

		$cached = get_transient('ifg-'.self::$slug.'-contacts');
		$sync = isset($_POST['sync']) ? (bool) $_POST['sync'] : '';

		if($sync === true || $cached == ''){

			$data    = array();
			$data[]  = array('Country', 'Contacts', array('role' => 'style'));
			$contact = new Infusionsoft_Contact();

			$today      = Infusionsoft_DataService::count($contact, array('DateCreated' => date('Y-m-d').'%', 'ReferralCode'=> 'added_by_infugrator_'.self::$slug.'_addon'));
			$yesterday  = Infusionsoft_DataService::count($contact, array('DateCreated' => date('Y-m-d',strtotime("-1 days")).'%', 'ReferralCode'=> 'added_by_infugrator_'.self::$slug.'_addon'));
			$this_month = Infusionsoft_DataService::count($contact, array('DateCreated' => date('Y-m-').'%', 'ReferralCode'=> 'added_by_infugrator_'.self::$slug.'_addon'));
			$last_month = Infusionsoft_DataService::count($contact, array('DateCreated' => date('Y-m-',strtotime('-1 month')).'%', 'ReferralCode'=> 'added_by_infugrator_'.self::$slug.'_addon'));
			$this_year  = Infusionsoft_DataService::count($contact, array('DateCreated' => date('Y-').'%', 'ReferralCode'=> 'added_by_infugrator_'.self::$slug.'_addon'));
			$last_year  = Infusionsoft_DataService::count($contact, array('DateCreated' => (date('Y')-1).'%', 'ReferralCode'=> 'added_by_infugrator_'.self::$slug.'_addon'));

			$get_stats = array(
				'Last Year'  => $last_year,
				'This Year'  => $this_year,
				'Last Month' => $last_month,
				'This Month' => $this_month,
				'Yesterday'  => $yesterday,
				'Today'      => $today,
			);

			foreach($get_stats as $key => $val){
				$data[] = array($key, $val, '#79BE37');
			}

			set_transient( 'ifg-'.self::$slug.'-contacts', $data, 60*60*24 );

		}else{
			$data = $cached;
		}

		$stats = array_merge($stats, array(
			self::$slug => array(
				'target' => 'ifg-'.self::$slug,
				'data'   => $data
			)
		));

		return $stats;
	}



	/**
	 * Enqueue scripts/styles in admin area
	 *
	 * @since  1.0.0
	 */
	public function admin_assets() {

		if( (isset($_GET['page']) && $_GET['page'] == 'gf_edit_forms') && (isset($_GET['subview']) && $_GET['subview'] == 'gf_infusionsoft') ){

			wp_deregister_style('select2');
			wp_deregister_script('select2');

			wp_enqueue_style('ifg-select2', plugin_dir_url(dirname(dirname(__FILE__))) . 'vendors/assets/css/select2.css', array(), '4.0.2', 'all' );
			wp_enqueue_script('ifg-select2', plugin_dir_url(dirname(dirname(__FILE__))) . 'vendors/assets/js/select2.min.js', array('ifg-pubsub'), '4.0.2', true);

			wp_enqueue_script( 'ifg-'.self::$slug.'-select', plugin_dir_url(__FILE__) . 'js/'.self::$slug.'-select.js', array('jquery', 'ifg-pubsub'), null, true );
		}

		if(isset($_GET['page']) && $_GET['page'] == 'ifg-stats'){

			wp_enqueue_script( 'ifg-'.self::$slug, plugin_dir_url(__FILE__) . 'js/'.self::$slug.'.js', array('jquery', 'ifg-pubsub'), null, true );
		}
	}



    /**
     * Add menu item to the Form Settings page menu
     *
     * @since 1.0.0
     * @param array $menu_items
     * @return array
     */
	public function set_form_settings_menu( $menu_items ) {

	    $menu_items[] = array(
			'name'  => 'gf_infusionsoft',
			'label' => __( 'Infusionsoft' )
        );

	    return $menu_items;
	}



	/**
	 * Get form fields
	 *
	 * @since  1.0.0
	 * @param  [integer] $form_id
	 * @return [array]
	 */
    public function get_form_fields( $form_id = null ){

    	$form = RGFormsModel::get_form_meta($form_id);
    	$fields = array();

        if(is_array($form["fields"])){

            foreach($form["fields"] as $field){

                if(isset($field["inputs"]) && is_array($field["inputs"]) && $field['type'] !== 'checkbox' && $field['type'] !== 'select'){

                    //If this is an address field, add full name to the list
                    if(RGFormsModel::get_input_type($field) == "address"){
                        $fields[] =  array($field["id"], GFCommon::get_label($field) . " (" . __("Full" , 'infugrator') . ")");
                    }

                    foreach($field["inputs"] as $input){
                        $fields[] =  array($input["id"], GFCommon::get_label($field, $input["id"]));
                    }
                }
                else if(empty($field["displayOnly"])){
                    $fields[] =  array($field["id"], GFCommon::get_label($field));
                }
            }
        }

        //Adding other fields (since v 1.5.8) - for example the results' quiz fields
        if( class_exists('GFFormsModel') ) {

            $extra_fields = GFFormsModel::get_entry_meta($form_id);

            foreach( $extra_fields as $key => $extra_field ) {
                $fields[] =  array( $key , $extra_field['label'] );
            }
        }

        return $fields;
    }



	/**
	 * Displaying content for our menu item when is selected
	 *
	 * @since 1.0.0
	 * @return html
	 */
	public function output_form_settings_tab() {
		require_once plugin_dir_path( __FILE__ ) . 'output-form-settings-tab.php';
	}



    /**
     * Generate the HTML output of values for mapping fields
     *
     * @since  1.0.0
     * @param  string $field_name
     * @param  string  $selected_field
     * @param  array $fields
     * @return string
     */
    public function output_mapped_fields($field_name, $selected_field, $fields = array()){

        $str = '<select name="ifg-'.self::$slug.'[fields]['.$field_name.']"><option value="">-</option>';

        if(is_array($fields)){
	        foreach($fields as $field){
	            $field_id = $field[0];
	            $field_label = $field[1];
	            $str .= '<option value="'.$field_id.'" '.selected(($field_id == $selected_field), true, false).'>' . $field_label . '</option>';
	        }
        }

        $str .= '</select>';

        return $str;
    }



    /**
     * Save the mapped fields
     *
     * @since  1.0.0
     */
    protected function save_mapped_fields(){

    	if ( isset($_POST['ifg-'.self::$slug.'-save'])  && isset($_POST['ifg-gravityforms']) && isset($_GET['id']) ) {

			$save_fields = array($_GET['id'] => $_POST['ifg-gravityforms']);

			if(is_array($this->mapped_fields) && count($this->mapped_fields) > 0){
				$save_fields = $this->mapped_fields;
				$save_fields[$_GET['id']] = $_POST['ifg-gravityforms'];
			}

			$this->utility->wp_option('update', 'ifg-'.self::$slug.'-fields', $save_fields);

			$this->mapped_fields = $save_fields; //refresh the mapped fields (kind of hack)

			$this->loader->add_action('ifg_save_gf_form_settings', $this, 'form_settings_message', 10);
	    }
    }



    /**
     * Generate the HTML output of success message
     *
     * @since  1.0.0
     * @return string
     */
    public function form_settings_message() {

        echo '<div class="ifg-success" style="margin: 0 0 20px;"><b>'.__('Fields were successfully saved.', 'infugrator').'</b></div>';
    }




    /**
     * Update the mapped fields when a form is deleted
     *
     * @since  1.0.0
     * @param  obj  $form_id
     */
	public function update_mapped_fields_on_form_deletion( $form_id ) {

	    if(is_array($this->mapped_fields) && count($this->mapped_fields) > 0){

	    	if(isset($this->mapped_fields[$form_id])){
	    		unset($this->mapped_fields[$form_id]);

	    		$this->utility->wp_option('update', 'ifg-'.self::$slug.'-fields', $this->mapped_fields);
	    	}
	    }

	}




	/**
     * Create a tooltip message
     *
     * @since  1.0.0
     * @param  array  $tooltips
     * @return array
     */
    public function add_tooltips($tooltips){

    	$tooltips['ifg_infusionsoft_apply_tags'] = __('<h6>Apply Tags</h6>Select what tags to be applied to the contact.', 'infugrator' );

    	return $tooltips;
    }



	/**
	 * Get data results in searching select field
	 *
	 * @since  1.0.0
	 * @return array
	 */
	public function search_tags(){

		if(!check_ajax_referer( 'ifg-security', 'security', false )){
			return;
		}

		$items = array();
		$results = null;
		$data = $_POST['remote'];

		if(isset($data['type'])){

			switch($data['type']){

				case 'tags':
					$results = $this->utility->get_tags();
					break;
			}

			if( is_array($results) && count($results) > 0 ){
				foreach($results as $result){
					$items[] = $result;
				}
			}
		}

		wp_send_json(array(
			'items' => $items
		));
	}




    /**
     * Insert contact in infusionsoft account
     *
     * @since 1.0.0
     * @param  array $entry
     * @param  obj $form
     */
	public function insert_contact( $entry, $form ) {

		$form_id = $entry['form_id'];

    	if(isset($this->mapped_fields[$form_id]['fields'])){

    		$fields = $this->mapped_fields[$form_id]['fields'];

    		//check if there are filled at least first name & email fields
			if( (isset($fields['FirstName']) && $fields['FirstName'] != '') && (isset($fields['Email']) && $fields['Email'] != '') ) {

    			$merge_vars = array();
    			$email = $this->utility->rgar( $entry, $fields['Email'] );

    			if(!is_email($email)){
    				return;
    			}

	    		foreach($fields as $fieldname => $val){

	    			$merge_vars[$fieldname] = $this->utility->rgar($entry, $val);
	    		}
	    		$merge_vars['ReferralCode'] = 'added_by_infugrator_'.self::$slug.'_addon';

	    		//add contact
    			$contactId = Infusionsoft_ContactService::addWithDupCheck($merge_vars, 'Email');

				// Set opt-in marketing status
				// Infusionsoft requires a "reason" for setting the opt-in marketing status
				$reason = get_bloginfo('name'). ' - Website Contact Form';
				// And allow them to receive email marketing
				Infusionsoft_EmailService::optIn($email, $reason);

				$tags = isset($this->mapped_fields[$form_id]['tags']) ? $this->mapped_fields[$form_id]['tags'] : '';
    			$apicallname = isset($this->mapped_fields[$form_id]['apicallname']) ? $this->mapped_fields[$form_id]['apicallname'] : '';
    			$integration = $this->settings->get('application', 'name');

				if(is_array($tags) && count($tags) > 0){
					foreach($tags as $tag){
						Infusionsoft_ContactService::addToGroup($contactId, $tag);
					}
				}

				//trigger campaign goal
				if($apicallname != ''){
					Infusionsoft_FunnelServiceBase::achieveGoal($integration, $apicallname, $contactId);
				}
    		}

    	}

	}


}
IFG_Gravity_Forms::instance();