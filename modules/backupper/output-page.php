			...
			<div class="widget">
				...
				<?php if($schedule != false && is_array($tables) && count($tables) > 0): ?>
					...
					<form action="" method="post">
						<input type="hidden" name="ifg-<?php echo self::$slug;?>-security" value="<?php echo wp_create_nonce( 'ifg-security' );?>">
				...
				<?php else:
					...
					echo sprintf(
						__('There are no backups yet. Please %s and choose what to bakup.', 'infugrator'),
						'<a href="'.admin_url('admin.php?page=ifg-settings').'">'.__('schedule a backup', 'infugrator').'</a>'
					); ?>
